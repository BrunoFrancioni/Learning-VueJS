<template>
  <div class="card my-5">
      <div class="card-body">
          ...
          <img :src="producto.imagen" width="320" height="150">
          <p class="text-center text-muted card-text display-4">$ {{ Number(producto.precio).toFixed() }}</p>
          <button @click="agregarCarro(producto)" :disabled="estaEnCarrito" class="btn btn-primary form-control">{{ estaEnCarrito ? 'Agregado' : 'Agregar al carrito' }}</button>
      </div>
  </div>
</template>
